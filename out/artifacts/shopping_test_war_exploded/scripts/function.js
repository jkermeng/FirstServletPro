...
function modify() {
    ...
    $.ajax({
        ...
        type: "POST",
        async: false,
        data: "newpwd=" + newpwd.value,
        success: function (e) {
            let parse = JSON.parse(e);
            console.log(parse);
            if (parse.code == 1) {
                newpwd.style.display = "inline";
...
function CheckItem(obj) {
    ...
    switch (obj.name) {
        case "newpwd":
            if (obj.value == "" || reg.test(obj.value) == false) {
                msgBox.innerHTML = "用户密码只能是字母开头和字母数字结尾，长度在4-16之间";
...
function phoneExist(phone) {
    var flag = true
    // var email = $("[name=mobile]").val()
    $.ajax({
        url: "CheckUser",// 请求的servlet地址
        type: "POST",// 请求方式
        data: "phone=" + phone,// 发送到服务器的数据
        async: true,
        dataType: "text",// 设置返回数据类型
        success: function (tests) {
            var test = JSON.parse(tests);
            if (test.code == 1) {
                var msgBox = document.getElementById("uphone");
                msgBox.style.display = "inline";
                msgBox.innerHTML = "该电话已经注册！";
                flag = false;
            } else {
                var msgBox = document.getElementById("uphone");
                msgBox.style.display = "block";
...
function checkValidateCode() {
    ...
    var flag = true;
    $.ajax({
        url: "/shopping_test/checkCode",// 请求的servlet地址
        type: "GET",// 请求方式
        async: false,
        data: "method=veryCode" + "&veryCode=" + veryCode,// 发送到服务器的数据
        dataType: "text",// 设置返回数据类型
